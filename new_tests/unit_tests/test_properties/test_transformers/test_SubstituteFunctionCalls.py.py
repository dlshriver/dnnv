...
from dnnv.properties.transformers import SubstituteFunctionCalls
...
def test_missing():
    ...
    with pytest.raises(
        ...
        SubstituteFunctionCalls().visit(FakeExpression())
    del FakeExpression


def test_Binary_no_func_call():
    transformer = SubstituteFunctionCalls()
...
def test_Binary_unsupported_expr():
    ...
    func_call = FunctionCall(Symbol("f"), (Symbol("a"),), {})
...
def test_Binary():
    ...
    func_call = FunctionCall(Symbol("f"), (Network("N")(Symbol("a")),), {})
...
def test_FunctionCall():
    ...
    func_call = FunctionCall(Constant(np.argmax), (Network("N")(Symbol("a")),), {})
    fake_network = lambda x: x
    fake_network.output_shape = ((1, 3),)
